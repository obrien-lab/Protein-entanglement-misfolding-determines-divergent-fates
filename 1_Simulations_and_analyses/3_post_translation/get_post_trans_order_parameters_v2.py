...
def get_Qbb():
    ...
    for i in range(n_traj):
        ...
        os.system("calc_native_contact_fraction.pl -i "+cor_file+" -d "+dom_def+" -s '"+sec_def+"' -t tmp/tmp.dcd -m 1 -o ./tmp/ > /dev/null")
        ...
        for j in range(rep_per_traj):
            fo = open(po_dir+'analysis/qbb/qbb_traj_'+str(i+1)+'_'+str(j+1)+'.dat', 'r')
            C = fo.readlines()
            fo.close()
            C = [C[k].strip().split() for k in range(1,len(C))]
            C = np.array(C, dtype=np.float32)
...
def get_entanglement():
    ...
    for i in range(n_traj):
        ...
        os.system('mkdir tmp')
        traj.save('tmp/tmp.dcd', force_overwrite=True)
        os.system("calc_entanglement_number_v2.pl -i "+cor_file+" -t tmp/tmp.dcd -o ./tmp/ > /dev/null")
        ...
        for j in range(rep_per_traj):
            G_list_1 = []
            f = open(po_dir+'analysis/G/G_traj_'+str(i+1)+'_'+str(j+1)+'.dat')
            C = f.readlines()
            f.close()
...
def get_K():
    ...
    for i in range(n_traj):
        for j in range(rep_per_traj):
            fo = open(po_dir+'analysis/chirality/K_traj_'+str(i+1)+'_'+str(j+1)+'.dat')
            C = fo.readlines()
            fo.close()
            C = [C[k].strip().split() for k in range(1,len(C))]
            C = np.array(C, dtype=np.float32)
            ...
            K_list.append(K_ts)
            print('  %s Traj %d_%d Done %d'%(mutant_type, i+1, j+1, K_list[-1].shape[0]))
    np.save('%s_K.npy'%(mutant_type), K_list)
