...
def gen_state_visualizion(state_id, native_psf, native_cor, state_psf, state_cor, native_struct, nc_length, if_entangled):
    ...
    sel = state_cor.topology.select("(residue 1 to %d) and (chainid 0)"%nc_length)
...
set viewplist {}
set fixedlist {}
set viewpoints([molinfo top]) {{{1 0 0 -25.9619} {0 1 0 -9.60361} {0 0 1 -1.88204} {0 0 0 1}} {{0.579449 0.00158709 -0.815005 0} {0.130733 0.986868 0.0948694 0} {0.804455 -0.16152 0.571631 0} {0 0 0 1}} {{'''+str(vmd_scale)+''' 0 0 0} {0 '''+str(vmd_scale)+''' 0 0} {0 0 '''+str(vmd_scale)+''' 0} {0 0 0 1}} {{1 0 0 -0.98} {0 1 0 0.14} {0 0 1 -1.29} {0 0 0 1}}}
...
if if_cluster:
    ...
    for i_div in range(len(NCL_div)):
        ...
        for i, cor in enumerate(cor_list):
            if i_div == len(NCL_div)-1:
                idx_list = np.where((NCL_list[i][:,0] >= start_ncl) & 
                                    (NCL_list[i][:,0] <= end_ncl) & 
                                    # (T_list[i][:,2] > 3))[0]
                                    (T_list[i][:,2] < 4))[0]
            else:
                idx_list = np.where((NCL_list[i][:,0] >= start_ncl) & 
...
for idx, div in enumerate(div_to_meta):
    if idx == len(NCL_div)-1:
        # start_ncl = max_length
        start_ncl = NCL_div[idx]
        end_ncl = max_length
    else:
        start_ncl = NCL_div[idx]
        end_ncl = NCL_div[idx+1]-1
    print("DIV [%d, %d]: %s"%(start_ncl, end_ncl, str(np.array(div)+1)))
meta_dtrajs = []
for traj in dtrajs:
...
for state_id in range(1, n_states+1):
    ...
    viz_G_gain = np.sum(visualiz_G[state_id-1][:2])
    viz_G_loss = np.sum(visualiz_G[state_id-1][2:4])
    viz_G_ch = visualiz_G[state_id-1][4]
...
if if_visualize:
    ...
    for state_id in range(1, n_states+1):
        ...
        if stage == 4:
            ...
            stage -= 1
        traj_name_prefix = 'rnc_l%d'%(ncl)
        if stage < 4:
            traj_name_prefix += '_stage_%d'%(stage)
        elif stage == 4:
            traj_name_prefix += '_ejection'
        elif stage == 5:
            traj_name_prefix += '_dissociation'
        traj_idx = trajid_list[traj_idx_0]
        psf_file = co_dir + '/traj/%d/rnc_l%d.psf'%(traj_idx+1, ncl)
        if frame_idx == len(NCL_list[traj_idx_0])-1:
            traj_file = co_dir + '/traj/%d/%s_final.cor'%(traj_idx+1, traj_name_prefix)
            frame_idx = 0
        elif T_list[traj_idx_0][frame_idx, 2] != T_list[traj_idx_0][frame_idx+1, 2]:
            traj_file = co_dir + '/traj/%d/%s_final.cor'%(traj_idx+1, traj_name_prefix)
            frame_idx = 0
        else:
            traj_file = co_dir + '/traj/%d/%s.dcd'%(traj_idx+1, traj_name_prefix)
            frame_idx_0 = 0
        ...
        gen_state_visualizion(state_id, native_psf_file, cor_file, psf_file, traj_cor, native_struct, ncl, if_entangled_list[state_id-1])
