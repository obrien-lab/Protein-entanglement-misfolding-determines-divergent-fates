...
try:
    for line in file_object:
        ...
        if line.startswith('sample_size'):
            ...
            continue
        if line.startswith('frame_offset'):
            words = line.split('=')
            frame_offset = int(words[1].strip())
            continue
...
def plot_state_map(x, y, z, nbins, cmap, ax, alpha=1, location='right', trap_mask=[], native_state_mask=[], mutant_type='', if_calc=True):
    ...
    cbar.set_ticklabels(['S%d'%(n+1) for n in range(n_state)])
    cax.tick_params(axis='both', labelsize='xx-small')
    if location == 'top':
...
for i_ax, mutant_type in enumerate(mutant_type_list):
    ...
    # Get Qbb trajectory
    qbb_list = np.load(mutant_type+'_QBB'+prefix_file+'_%d.npy'%frame_offset, allow_pickle=True)[:,:,-1]
    ...
    G_list_0 = list(np.load(mutant_type+'_G_%d.npy'%frame_offset, allow_pickle=True)[mask])
    ...
    for zi in range(data_list[2].shape[1]):
        ...
        for zj in range(data_list[2].shape[0]):
            if data_list[2].mask[zj,zi]:
                ...
                sm.append('S%d'%(data_list[2][zj,zi]+1))
        SM_data.append(sm)
...
for state_id in range(1, n_states+1):
    ...
    G_center = coarse_state_centers[state_id-1][1]
    ...
    if (G_avg > visualiz_threshold) or (G_center > visualiz_threshold):
        if max_idx == 0:
