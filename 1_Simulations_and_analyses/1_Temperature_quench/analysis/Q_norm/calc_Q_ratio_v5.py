...
for idx, data_list in enumerate([YU_E_list, NU_E_list]):
    ...
    p_nent_prot_list = [[] for ln in label_postfix_list]
    boot_ent_prot_data = [[] for ln in label_postfix_list]
    boot_nent_prot_data = [[] for ln in label_postfix_list]
    for uid in data_list:
        work_dir = '../../' + label_list[idx] + '/' + uid + '/analysis/'
        n_traj = np.nan
        Q_ent_list = [[] for ln in label_postfix_list]
        Q_nent_list = [[] for ln in label_postfix_list]
        Q_native_list = [[] for ln in label_postfix_list]
        if os.path.exists(work_dir):
            if os.path.exists(work_dir+'msm_data.npz'):
                ...
                # Get Q trajectory
                Q_list = np.load(work_dir+'Tq_QBB_0.npy', allow_pickle=True)[mask]
                ...
                for iframe in range(len(label_postfix_list)):
                    ...
                    for it, md in enumerate(meta_dtrajs):
                        ...
                        if len(idx_list) > 0:
                            native_Q.append(np.mean(Q_list_0[it, idx_list]))
                        idx_list = np.where(np.isin(md, ent_idx))[0]
                        if len(idx_list) > 0:
                            ent_Q.append(np.mean(Q_list_0[it, idx_list]))
                        idx_list = np.where(np.isin(md, nent_idx))[0]
                        if len(idx_list) > 0:
                            nent_Q.append(np.mean(Q_list_0[it, idx_list]))
                    
                    Q_ent_list[iframe] = ent_Q
                    Q_nent_list[iframe] = nent_Q
                    Q_native_list[iframe] = native_Q                    

        pd_data.append([uid, n_traj])
        raw_data[uid] = [[Q_native_list, Q_ent_list, Q_nent_list]]
        # bootstrap
        boot_native_data = []
        boot_ent_data = []
        boot_nent_data = []
        for iframe in range(len(label_postfix_list)):
            boot_native_data.append(bootstrap(np.mean, np.array(Q_native_list[iframe]), n_boot))
            boot_ent_data.append(bootstrap(np.mean, np.array(Q_ent_list[iframe]), n_boot))
            boot_nent_data.append(bootstrap(np.mean, np.array(Q_nent_list[iframe]), n_boot))
        raw_data[uid].append([boot_native_data, boot_ent_data, boot_nent_data])
        
        # compute mean and 95% CI
        for iframe in range(len(label_postfix_list)):
            if len(Q_native_list[iframe]) == 0:
                p_ent_str = ''
                ...
                if len(Q_ent_list[iframe]) == 0:
                    p_ent_str = ''
                    p_ent_prot_list[iframe].append(np.nan)
                    boot_ent_prot_data[iframe].append(np.nan*np.ones(n_boot))
                else:
                    ratio = np.mean(Q_ent_list[iframe]) / np.mean(Q_native_list[iframe])
                ...
                if len(Q_nent_list[iframe]) == 0:
                    p_nent_str = ''
                    p_nent_prot_list[iframe].append(np.nan)
                    boot_nent_prot_data[iframe].append(np.nan*np.ones(n_boot))
                else:
                    ratio = np.mean(Q_nent_list[iframe]) / np.mean(Q_native_list[iframe])
...
np.save('Q_ratio_raw_data_G%.3f_v5.npy'%(G_cutoff), raw_data)
...
with pd.ExcelWriter('./Q_ratio_G%.3f_v5.xlsx'%(G_cutoff)) as writer:
...
for i_ax in range(6):
    ...
    for j in range(len(label_list)):
        data_list = data_df_list[j].dropna().iloc[:,i_ax-6].to_list()
    ...
    ax.bar(np.arange(len(label_list)), bar_data, width=0.8, bottom=0, yerr=bar_yerr,
           ...
           alpha=0.2, zorder=10)
    y_label = data_df_list[0].columns[i_ax-6].split('(')[0].strip()
    if len(y_label) > 25:
        words = y_label.split()
        words[0] += '\n'
        y_label = ' '.join(words)
    ...
    ax.set_ylim([0,1])
    ax.set_title(label_postfix_list[int(i_ax/2)-3])

fig.savefig('./Q_ratio_G%.3f_v5.svg'%(G_cutoff))

# plot time series
fig = plt.figure(figsize=(6, 3))
plt.subplots_adjust(left=0.2, wspace=0.4, hspace=0.5, top=0.9, bottom=0.2)
label_list = ['YU & E', 'NU & E']
color_list = ['red', 'orange']
for i_ax in range(4):
    ax = fig.add_subplot(2,2,1+i_ax)
    if i_ax % 2 == 0:
        for j in range(len(label_list)):
            data_list = data_df_list[j].dropna().iloc[-1,2+int(i_ax/2)::2].to_list()
            mean_list = np.array([float(d.split()[0]) if d != '' else np.nan for d in data_list])
            lb_list = np.array([float(d.split()[1][1:-1]) if d != '' else np.nan for d in data_list])
            ub_list = np.array([float(d.split()[2][:-1]) if d != '' else np.nan for d in data_list])
        ...
        # p-values vs t
        data_list = data_df_list[-1].dropna().iloc[:,1+int(i_ax/2)::2].to_numpy()
...
fig.savefig('./Q_ratio_vs_t_G%.3f_v5.svg'%(G_cutoff))
