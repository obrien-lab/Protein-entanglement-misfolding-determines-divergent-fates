...
last_nframes_list = np.array([-500, -300, -100]) # ns
G_cutoff = 0.005
n_boot = int(1e5)
...
for idx, data_list in enumerate([YU_E_list, NU_E_list]):
    ...
    for uid in data_list:
        ...
        n_traj = np.nan
        Q_ent_list = [[] for ln in last_nframes_list]
        Q_nent_list = [[] for ln in last_nframes_list]
        Q_native_list = [[] for ln in last_nframes_list]
...
np.save('Q_ratio_raw_data_G%.3f_v4.npy'%(G_cutoff), raw_data)
...
with pd.ExcelWriter('./Q_ratio_G%.3f_v4.xlsx'%(G_cutoff)) as writer:
...
fig.savefig('./Q_ratio_G%.3f_v4.svg'%(G_cutoff))
