...
NU_NE_list = np.loadtxt('../../../gen_parameter/NU_NE.txt', dtype=str)
...
for idx, data_list in enumerate([YU_E_list, NU_NE_list, NU_E_list]):
    pd_data = []
    p_nn_prot_list = [[] for ln in last_nframes_list]
    p_ent_prot_list = [[] for ln in last_nframes_list]
    p_nnne_prot_list = [[] for ln in last_nframes_list]
    boot_nn_prot_data = [[] for ln in last_nframes_list]
    boot_ent_prot_data = [[] for ln in last_nframes_list]
    boot_nnne_prot_data = [[] for ln in last_nframes_list]
    for uid in data_list:
        work_dir = '../../' + label_list[idx] + '/' + uid + '/analysis/'
        n_traj = np.nan
        p_ent_list = [[] for ln in last_nframes_list]
        p_nn_list = [[] for ln in last_nframes_list]
        p_nnne_list = [[] for ln in last_nframes_list]
...
np.save('raw_data_G%.3f_v5.npy'%(G_cutoff), raw_data)
...
with pd.ExcelWriter('./misfolding_propensity_G%.3f_v5.xlsx'%(G_cutoff)) as writer:
...
#label_list = ['Young ubiquitinated\n& entangled proteins', 
#              'Non-ubiquitinated\n& non-entangled proteins',
#              'Non-ubiquitinated\n& entangled proteins']
for i_ax in range(9):
    ...
    ax.set_ylim([0,1])
    ax.set_title(label_postfix_list[int(i_ax/3)])

fig.savefig('./misfolding_propensity_G%.3f_v5.svg'%(G_cutoff))
